...
class DownpourServer(Server):
    def __init__(self):
        ...
        self.server_.downpour_server_param.service_param.server_class = "AbacusBrpcPsServer"
        self.server_.downpour_server_param.service_param.client_class = "AbacusBrpcPsClient"
        self.server_.downpour_server_param.service_param.service_class = "AbacusPsService"
        self.server_.downpour_server_param.service_param.start_server_port = 0
    ...
    def add_sparse_table(self, table_id, learning_rate,
        ...
        table.table_class = "AbacusSparseTable"
        table.type = pslib.PS_SPARSE_TABLE
        table.accessor.accessor_class = "AbacusFeatureValueAccessor"
    ...
    def add_dense_table(self, table_id, learning_rate, params_grads):
        #param_var, grad_var):
        table = self.server_.downpour_server_param.downpour_table_param.add()
        table.table_id = table_id
        table.table_class = "AbacusDenseTable"
        table.type = pslib.PS_DENSE_TABLE
        table.accessor.accessor_class = "AbacusDenseValueAccessor"
        ...
        fea_dim = 0
        for param in params_grads:
            fea_dim += reduce(lambda x, y: x * y, param[0].shape, 1)
...
class DownpourWorker(Worker):
    ...
    def add_dense_table(self, table_id, learning_rate, params_grads):
        ...
        table.table_id = table_id
        table.dense_variable_name.extend([p[0].name for p in params_grads])
        table.dense_gradient_variable_name.extend([g[1].name for g in params_grads])
